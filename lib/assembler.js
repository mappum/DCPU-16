            ...
            for( i = 0; i < line.length; i++) {
                ...
                if(!isWhitespace(c)) {
                    if(c === ';') {
                    	...
                    	if(token.charAt(0) === ':') {
                    		output.labels[token.substr(1).toLowerCase()] = output.instructions.length;
                    		i += token.length;
                    	} else if(token.charAt(token.length - 1) === ':') {
                    		output.labels[token.substr(0, token.length - 1).toLowerCase()] = output.instructions.length;
                    		i += token.length;
                        } else if(!op) {
                            op = token;
                            i += op.length;
    ...
    compile: function(code) {
        ...
        function parse(arg) {
            ...
            //literals/pointers
                ...
                if(!pointer && (value <= 0x1e || value === 0xffff) && OPCODES[op].code !== undefined) {
