    ...
    step: function() {    	
        ...
        switch (insn.opcode) {
            ...
            case 0x11:
            	if(bVal & aVal) {
            		var opcode;
                    do{ opcode = this.nextInstruction().opcode; }
            ...
            case 0x220:
            	if(this._devices[aVal]) {
            		this.set('b', this._devices[aVal].id >> 16);
            		this.set('a', this._devices[aVal].id & 0xffff);
            		this.set('c', this._devices[aVal].version);
            		this.set('y', this._devices[aVal].manufacturer >> 16);
            		this.set('x', this._devices[aVal].manufacturer & 0xffff);
            	} else {
            		this.set('a', 0);
            		this.set('b', 0);
            		this.set('c', 0);
